var page = "/ajax.php";
var loading_img =
  "<img style='vertical-align:middle' src='" + imageurl + "/ajax-loader.gif'>";
...
function check_remote_url() {
  ...
  // Obtenir toutes les URLs et les séparer par des virgules
  var urls = $("#remote_file_url")
    .val()
    .split(",")
    .map((url) => url.trim());
  var ajaxCall = null;
  ...
  if (!urls || urls.length === 0 || urls[0] === "undefined") {
    $("#error_msgs").html(
      '<div class="alert alert-danger" role="alert">Veuillez entrer des URLs valides</div>'
    );
  ...
  processNextFile(counter, urls, ajaxCall);
}

// Traiter chaque URL
function processNextFile(counter, urls, ajaxCall) {

  console.log(urls[counter]);
  if (counter < urls.length) {
    var file = urls[counter];
    var file_name = getName(file.trim());
    ...
    ajaxCall = $.ajax({
      ...
      beforeSend: function () {
        ...
        $("#loading").html(
          '<div style="float:left;display:inline-block;"><img src="' +
            imageurl +
            '/ajax-loader.gif"></div><div style="float:left;line-height:16px;padding-left:5px;">' +
            lang.remoteUploadFile +
      ...
      success: function (data) {
        if (data.error) {
          ...
          if(urls.length == 1) alert(data.error);
        }
        $("#loading").html("");
        ...
        $.post(
          ...
          function (data) {
            ...
            $("#cbSubmitUpload" + remoteObjID)
              .before(
                '<span id="updateVideoDataLoading" style="margin-right:5px"></span>'
              )
              .attr("disabled", "")
              .attr("value", lang.saveData)
              .attr(
                "onClick",
                "doUpdateVideo('#uploadForm" +
                  remoteObjID +
                  "','" +
                  remoteObjID +
                  "')"
              ...
              .after(
                '<input type="hidden" name="updateVideo" value="yes" id="updateVideo" />'
              );
              ...
              .tagit({
                ...
                beforeTagAdded: function (event, info) {
                  if (info.tagLabel.length <= 2) {
                    if (!alert_shown) {
                      alert_shown = true;
                      if(urls.length == 1) alert(tag_too_short);
                    }
                    return false;
                  }
                  alert_shown = false;
            ...
            // After completing the current file, process the next one
          //   counter++;
            console.log("SHOW COUNTER : " + counter);
            if (counter+1 < urls.length) {
              processNextFile(counter+1, urls, ajaxCall);
            }else{
              alert(window.onrrad);
...
function delete_video(obj, id, msg, url) {
...
function construct_url(jsArr) {
  ...
  if (Seo == "yes") {
    ...
    url =
      "#!?item=" + jsArr[2] + "&type=" + jsArr[0] + "&collection=" + jsArr[1];
    window.location.hash = url;
  }
}
...
function getDetails(obj) {
  var forms = getInputs(obj),
    ParamArray = new Array(forms.length);
...
function loadObject(currentDOM, type, objID, container) {
  ...
  $.ajax({
    ...
    beforeSend: function () {
      obj.find("img").animate({ opacity: 0.5 });
      $("#" + object["container"]).animate({ opacity: 0.5 });
    },
    success: function (data) {
      if (data["error"]) {
...
function getAllComments(
  ...
  $("#userCommentsList").html(
    "<div style='padding:5px 0px;'>" + loading + "</div>"
  );
  $.ajax({
    type: "POST",
    url: page,
    ...
    success: function (comments) {
      ...
      if (visual_editor_comments_enabled) {
        Array.from(
          document.querySelectorAll("#userCommentsList .commented-txt p")
        ...
        success: function (response) {
          ...
          data.videos.forEach(function (video) {
            if (video.status.toLowerCase() == "processing") {
              //update %
              var process_div = $(
                '.processing[data-id="' + video.videoid + '"]'
              );
              //if process don't exist : get thumb + process div
              if (process_div.length == 0) {
                $('.item-video[data-id="' + video.videoid + '"]').html(
                  video.html
                );
              } else {
              ...
              if (
                displayType === "view_channel_player" &&
                data.player !== undefined &&
                data.player.id == video.videoid
              ) {
